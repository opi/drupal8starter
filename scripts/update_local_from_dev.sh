...
drush -y rsync @dev:%files/ @local:%files
#scp -r root@dev-machine:$REMOTE_DOCUMENT_ROOT/sites/default/files/* $DOCUMENT_ROOT/sites/default/files/
...
drush @dev config-export -y
drush rsync -y @dev:%config/ @local:%config
drush config-import
